...
export default function partB(): void {
  ...
  for (const history of input) {
    ...
    for (let i = historyOfHistorys.length - 2; i >= 0; i--) {
      const firstNumber = historyOfHistorys[i + 1][0];
      const nextfirstNum = historyOfHistorys[i][0];
      const newNextNum = nextfirstNum - firstNumber;
      historyOfHistorys[i].unshift(newNextNum);
      if (i === 0) {
